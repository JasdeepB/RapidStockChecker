...
const alarm = 'alarmXBOXS';
const alarmType = 'alarmTypeXBOXS';
...
signalrConnection.start().then(res => {
    signalrConnection.invoke("JoinGroup", "XBOXSeriesS")
        .catch(err => {
            console.log(err);
        });
}).catch(err => {
...
signalrConnection.on("XBOXSeriesSAlertTrigger", function (products) {
